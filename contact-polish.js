/* TRANSMIND CONTACT FINAL v4
   Exact order: area -> description -> WhatsApp 1 -> WhatsApp 2 -> emails -> payment.
*/
(function(){
  'use strict';
  const items=[
  ...
  function boot(){
    ...
    style.textContent='#kontak .tm-wa-group-final{margin:12px 0 16px!important}#kontak .tm-wa-line-final{display:flex!important;align-items:center!important;gap:8px!important;margin:7px 0!important;line-height:1.35!important}#kontak .tm-wa-line-final a{display:inline-flex!important;align-items:center!important;gap:8px!important;text-decoration:none!important;white-space:nowrap!important}#kontak .tm-wa-icon-final{width:19px;height:19px;flex:0 0 19px;display:inline-block}#kontak .tm-wa-line-final a:hover{opacity:.86}@media(max-width:650px){#kontak .tm-wa-group-final{margin:10px 0 14px!important}#kontak .tm-wa-line-final{margin:6px 0!important}#kontak .tm-wa-icon-final{width:18px;height:18px;flex-basis:18px}}';
    document.head.appendChild(style);

    c.querySelectorAll('.tm-wa-group-final').forEach(x=>x.remove());
    c.querySelectorAll('p').forEach(p=>{
      const digits=(p.textContent||'').replace(/\D/g,'');
      if(items.some(i=>digits.includes(i.display)||digits.includes(i.wa)))p.remove();
    });

    const group=document.createElement('div');group.className='tm-wa-group-final';
    items.forEach(item=>{
      const row=document.createElement('div');row.className='tm-wa-line-final';
    ...
    const nodes=Array.from(c.querySelectorAll('p'));
    const email=nodes.find(p=>/Email\s*(Booking|Admin)/i.test(p.textContent||''));
    if(email) c.insertBefore(group,email); else c.appendChild(group);
  }

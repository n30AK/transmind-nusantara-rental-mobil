...
async function render(days=7){
 ...
 try{
  ...
  el.innerHTML=css+
   ...
   '<div class="f2-card f2-section"><h3>🚀 F3 Autonomous Opportunity Queue</h3><div class="f2-muted" style="margin-bottom:10px">Prioritas dihitung dari sinyal first-party yang tersedia: demand, conversion gap, geo/intent concentration, dan freshness. Queue ini tidak membuat traffic atau ranking palsu.</div><div class="f2-table"><div class="f2-row f2-head"><span>Priority</span><span>Signal</span><span>Impact</span><span>Recommended move</span></div>'+unique.map((o,i)=>{const score=Math.max(20,100-i*7);return '<div class="f2-row"><span><b>#'+(i+1)+' · '+score+'</b></span><span>'+esc(o.title)+'</span><span class="f2-pill">'+esc(o.why)+'</span><span>'+esc(o.action)+'</span></div>';}).join('')+(unique.length?'':'<div class="f2-muted">Queue menunggu sinyal demand yang cukup.</div>')+'</div><div class="f2-actions" style="margin-top:10px"><button type="button" class="f2-pill" id="tm-f3-refresh">Refresh intelligence</button><button type="button" class="f2-pill" id="tm-f3-copy">Copy action queue</button></div></div>'+'<div class="f2-card f2-section"><h3>🎯 Immediate Conversion Actions</h3><div class="f2-actions"><a class="primary" href="https://transmindnusantararentalmobil.co.id/#booking">Buka Booking</a><a href="https://wa.me/628816654141" target="_blank" rel="noopener">WhatsApp Admin</a><a href="https://transmindnusantararentalmobil.co.id/rental-mobil-jakarta.html">Jakarta</a><a href="https://transmindnusantararentalmobil.co.id/rental-mobil-bekasi.html">Bekasi</a><a href="https://transmindnusantararentalmobil.co.id/rental-mobil-bogor.html">Bogor</a><a href="https://transmindnusantararentalmobil.co.id/rental-mobil-depok.html">Depok</a><a href="https://transmindnusantararentalmobil.co.id/rental-mobil-tangerang.html">Tangerang</a></div></div>';
 }catch(e){el.innerHTML='<div class="notice bad">SEO F2 tidak dapat membaca data: '+esc(e.message)+'</div>';}
 finally{setTimeout(()=>{window.TRANSMIND_BOOKING_GROWTH?.render?.();window.TRANSMIND_AI_CUSTOMER_CARE_GROWTH?.boot?.();},60);}
}
function install(){if(window.__TM_SEO_F2_INSTALLED)return;window.__TM_SEO_F2_INSTALLED=true;window.TRANSMIND_SEO_F1={render};window.addEventListener('tm-seo-refresh',e=>render(Number(e.detail?.days||7)));document.addEventListener('click',e=>{if(e.target.id==='tm-f3-refresh')render(7);if(e.target.id==='tm-f3-copy'){const q=[...document.querySelectorAll('#seoBody .f2-table .f2-row')].slice(-11).map(x=>x.innerText).join('\n');navigator.clipboard?.writeText(q);e.target.textContent='Copied';setTimeout(()=>e.target.textContent='Copy action queue',1200);}});document.addEventListener('click',e=>{const b=e.target.closest('[data-seo]');if(b)setTimeout(()=>render(Number(b.dataset.seo||7)),50);});}

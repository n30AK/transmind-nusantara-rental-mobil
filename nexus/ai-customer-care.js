...
function uniqueCandidates(d){
 ...
 d.comms.filter(x=>String(x.direction)==='outbound'&&String(x.channel)==='whatsapp').forEach(x=>{
  const phone=normPhone(x.recipient||x.metadata?.phone||''); if(!phone)return;
  const key=phone; const old=by.get(key)||{phone,signals:0,tags:[],intent:'booking'};
  old.customer_id=old.customer_id||x.customer_id; old.latest=old.latest||x.created_at; old.last_seen_at=old.last_seen_at||x.created_at; old.name=old.name||x.metadata?.lead_name; old.signals+=1; by.set(key,old);
 });
 d.tasks.filter(x=>String(x.status)==='open'&&x.metadata?.phone).forEach(x=>{
  const phone=normPhone(x.metadata.phone); if(!phone)return;
  const old=by.get(phone)||{phone,signals:0,tags:[],intent:'booking'}; old.customer_id=old.customer_id||x.customer_id; old.latest=old.latest||x.created_at; old.last_seen_at=old.last_seen_at||x.created_at; old.name=old.name||x.metadata?.lead_name; old.signals+=1; by.set(phone,old);
 });
 return [...by.values()];
}
...
async function render(){
 const el=document.getElementById('seoBody'),db=client();if(!el||!db)return; if(el.querySelector('.tm-care-mounted'))return;
 try{
...
document.addEventListener('tm-seo-refresh',()=>setTimeout(render,80));

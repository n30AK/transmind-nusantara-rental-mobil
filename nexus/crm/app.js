(() => {
  ...
  async function load(){
    ...
    $('metrics').innerHTML=[
      metric('New Leads',summary.new_leads||0,'baru masuk'),
      metric('Follow-up Due',summary.followup_due||0,'perlu ditindak'),
      metric('Quoted',summary.quoted||0,'sudah penawaran'),
      metric('Confirmed',summary.confirmed||0,'siap dijalankan'),
      metric('Paid',summary.paid||0,'sudah bayar')
    ].join('');
    render();
  }
  function filtered(){
    ...
    return rows.filter(r=>{
      const hay=[r.customer_name,r.customer_phone,r.booking_code,r.service,r.title].join(' ').toLowerCase();
      if(q&&!hay.includes(q))return false;
      if(stage&&r.pipeline_stage!==stage)return false;
      if(scope==='due' && !(r.next_followup_at && new Date(r.next_followup_at).getTime()<=now))return false;
      if(scope==='priority' && !['high','urgent','critical'].includes(String(r.priority||'').toLowerCase()))return false;
      return true;
    });
  }
  function render(){
    ...
    $('queue').innerHTML=list.length?list.map(r=>{
      const p=String(r.priority||'').toLowerCase(), cls=['high','urgent','critical'].includes(p)?' high':'';
      return `<div class="row" data-id="${esc(r.task_id)}"><div><div class="customer">${esc(r.customer_name||'Tanpa nama')}</div><small>${esc(r.customer_phone||'')} · ${esc(r.booking_code||'Tanpa booking')}</small></div><div><span class="priority${cls}">${esc(r.priority||'normal')}</span><div class="muted">${esc(r.service||'')} · ${fmtDate(r.start_date)}</div></div><div><select class="stage-select" data-stage="${esc(r.task_id)}">${stages.map(s=>`<option ${s===r.pipeline_stage?'selected':''}>${s}</option>`).join('')}</select></div><div class="row-actions"><button class="mini" data-open="${esc(r.task_id)}">Detail</button>${r.customer_phone?`<a class="mini gold" target="_blank" rel="noopener" href="https://wa.me/${wa(r.customer_phone)}?text=${encodeURIComponent('Halo '+(r.customer_name||'')+', kami dari Transmind Nusantara. Kami siap membantu kebutuhan rental Anda.')}">WhatsApp</a>`:''}</div></div>`;
    }).join(''):`<div class="empty">Tidak ada task sesuai filter.</div>`;
    document.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>select(b.dataset.open));
    document.querySelectorAll('[data-stage]').forEach(s=>s.onchange=()=>changeStage(s.dataset.stage,s.value));
    if(selected)select(selected.task_id,true);
  }
  function select(id,silent=false){
    const r=rows.find(x=>String(x.task_id)===String(id)); if(!r)return; selected=r;
    const phone=wa(r.customer_phone);
  ...
  async function changeStage(id,stage){
    const r=rows.find(x=>String(x.task_id)===String(id));if(!r)return;
    let reason=r.lost_reason||null;
    if(stage==='LOST'){reason=prompt('Alasan lost (wajib diisi):',reason||'');if(!reason){render();return;}}
    await update(id,stage,r.next_followup_at,r.quote_value,reason);
  }
  async function update(id,stage,next,quote,lost){
    $('connection').textContent='● SAVING';
    const {error}=await sb.rpc('nexus_crm_update_pipeline',{p_task_id:id,p_stage:stage,p_next_followup_at:next||null,p_quote_value:quote??null,p_lost_reason:lost||null});
    if(error){$('connection').textContent='● ERROR';alert(error.message);return;}
    await load(); select(id,true); $('connection').textContent='● LIVE';
  }
  async function logContact(id,outcome,next){
    ...
    $('detailMsg').textContent='Aktivitas tersimpan dan audit tercatat.';$('detailMsg').className='msg ok';
    await load(); if(selected)select(id,true); $('connection').textContent='● LIVE';
  }
  async function signIn(e){
    e.preventDefault();msg('Authenticating…');
    const {data,error}=await sb.auth.signInWithPassword({email:$('email').value.trim(),password:$('password').value});
    if(error){msg('Login gagal. Periksa kredensial atau akses NEXUS.',true);return;}
    session=data.session;
    try{await roleAllowed();showApp(true);msg('');$('connection').textContent='● LIVE';await load();}
    catch(err){await sb.auth.signOut();session=null;showApp(false);msg('Akun belum memiliki akses CRM NEXUS.',true);console.error(err);}
  }
  async function boot(){
    $('loginForm').onsubmit=signIn;$('logout').onclick=async()=>{await sb.auth.signOut();session=null;showApp(false)};
    ['search','stage','scope'].forEach(id=>$(id).oninput=render);$('refresh').onclick=load;
    const {data}=await sb.auth.getSession();
    if(data.session){session=data.session;try{await roleAllowed();showApp(true);await load()}catch(e){await sb.auth.signOut();session=null;}}
    sb.auth.onAuthStateChange((_e,s)=>{session=s;if(!s)showApp(false)});

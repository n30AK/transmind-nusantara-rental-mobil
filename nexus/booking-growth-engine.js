/* TransMind Nexus — Booking 50/day Growth Engine v2
 * Booking truth comes from the authoritative bookings table.
 * Analytics events are funnel telemetry, never the booking ledger.
 */
(()=>{'use strict';
...
async function render(){
 const el=document.getElementById('seoBody'),d=db();
 if(!el||!d||el.dataset.tmGrowth==='1')return;
 el.dataset.tmGrowth='1';
 try{
  ...
  const rows=ev.data||[];
  const authoritative=(bookRows.data||[]).filter(x=>!['dibatalkan','cancelled','canceled','refund','refunded'].includes(String(x.status||'').trim().toLowerCase()));
  const telemetry=rows.filter(x=>/^(booking_success|booking_created)$/.test(String(x.event_type||'')));
  const telemetryKeys=new Set(telemetry.map(x=>String(x.booking_id||x.booking_code||((x.visitor_session_id||'')+'|'+String(x.occurred_at||'').slice(0,16)))));
  const bookings=authoritative.length||telemetryKeys.size;
  const cta=count(rows,'booking_cta_click'),wa=count(rows,'whatsapp_click'),starts=count(rows,'booking_start');
  const target=50,gap=Math.max(0,target-bookings);
  ...
  const truth=authoritative.length?'master booking':'telemetry fallback';
  const action=bookings>=target?'Pertahankan pola yang terbukti menghasilkan booking dan tingkatkan kapasitas secara bertahap.':'Prioritaskan kebocoran '+leak+'. Booking dihitung dari transaksi nyata; telemetry hanya menjadi fallback ketika ledger tidak dapat dibaca.';
  const html='<style id="tm-growth-style">.tm-g{margin-top:14px}.tm-g-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.tm-g-card{background:linear-gradient(145deg,#171a21,#101319);border:1px solid #292f39;border-radius:15px;padding:16px}.tm-g-num{font-size:26px;font-weight:900;margin-top:5px}.tm-g-note{font-size:11px;color:#8e949f;line-height:1.5}.tm-g-hot{color:#e5c15a}.tm-g-danger{color:#e87878}.tm-g-ok{color:#65d49a}@media(max-width:850px){.tm-g-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:520px){.tm-g-grid{grid-template-columns:1fr}}</style>'+
  '<div class="tm-g"><div class="tm-g-card"><div class="tm-g-note">BOOKING NYATA · 24 JAM</div><div class="tm-g-num '+(bookings?'tm-g-ok':'tm-g-danger')+'">'+bookings+'</div><div class="tm-g-note">'+truth+'</div></div>'+
  ...
  el.insertAdjacentHTML('beforeend','<div class="notice bad tm-g">Booking Growth Engine: '+esc(e.message)+'</div>');
 }
}

(() => {
  const mount = () => {
    ...
    const client = window.__nexusSupabase || (window.getTransmindSupabaseClient?window.getTransmindSupabaseClient():window.supabase.createClient(window.NEXUS_CONFIG.supabaseUrl,window.NEXUS_CONFIG.supabaseAnonKey));

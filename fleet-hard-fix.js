/* TRANSMIND FLEET STORAGE RENDERER v3 */
(function(){
  'use strict';
  const MAX=24,BUCKET='vehicle-images';
  function client(){
  ...
  function updateStatus(){
    const b=document.querySelector('#cars'),s=document.querySelector('#fleetStatus');
    if(s&&b)s.textContent='Menampilkan '+b.querySelectorAll('.car').length+' unit armada.';
  }
  function render(sb,vehicles){
    const box=document.querySelector('#cars');if(!box)return;
    box.innerHTML='';
    vehicles.filter(v=>v&&v.id&&v.name&&v.image_path).slice(0,MAX).forEach(v=>{
      ...
      const img=document.createElement('img');img.src=imageUrl(sb,v.image_path);img.alt=v.name+' — Transmind Nusantara Rental Mobil';img.loading='lazy';img.decoding='async';
      img.onerror=()=>{card.remove();updateStatus()};
      photo.appendChild(img);card.appendChild(photo);
  ...
  async function boot(){
    ...
    try{
      const r=await sb.from('vehicles').select('id,name,category,capacity,active,image_path,sort_order').eq('active',true).not('image_path','is',null).order('sort_order',{ascending:true,nullsFirst:false}).order('name',{ascending:true}).limit(MAX);
